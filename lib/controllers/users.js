...
module.exports = Router()
  ...
  .post('/state', async (req, res, next) => {
    try {
      const state = await userState.insert(req.body.actionStateId);
      res.json(state);
    } catch (e) {
      next(e);
    }
  })
  .delete('/state', async (req, res, next) => {
    try {
  ...
  .get('/state/:id', async (req, res, next) => {
    try {
      const state = await userState.getById(req.params.id);
